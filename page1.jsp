...
<div class="box box-default ">
	...
	<div class="box-body">
<div class="EntryUnit ">
	<html:hidden property="enu13Key" styleId="enu13Key"></html:hidden>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu13"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<c:set var="btFieldError"><html:errors property="fld60"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld60" class="control-label col-md-2">Nombre de Usuario</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld60" styleClass="wr-submitButtons:ln27  form-control " property="fld60" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld60"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld61"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld61" class="control-label col-md-2">Contrase&ntilde;a</label>
					<div class="controls col-md-10">
					<input type="password" autocomplete="off" id="fld61" class="wr-submitButtons:ln27 form-control " name="fld61"/>
					<c:set var="btFieldError"><html:errors property="fld61"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
					 <button title="Aceptar" onclick="$('#page1FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln27" name="button:ln27" type="submit" value="Aceptar">
			Aceptar</button>
		</div>
	</div></div>
</div>	
