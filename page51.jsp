...
<body class="skin-blue fixed">
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page51.do${wrAjax ? '#!ajax=true' : ''}" styleId="page51FormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page51"/>
				<input type="hidden" name="ln29" value="<webratio:Link link="ln29"/>">
...
<c:if test="${not(empty mciu5) and (mciu5.dataSize gt 0)}">
	...
	<div class="box-body">
		<div class="plain ">
			<div class="plain MultiChoiceIndexUnit">
				<c:set var="btFieldError"><html:errors property="mciu5Checked"/></c:set>
				<c:if test="${not (empty btFieldError)}">
					<div class="alert alert-danger alert-dismissable">
						<span class="glyphicon glyphicon-ban-circle"></span>
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
						<html:errors property="mciu5Checked"/>
					</div>
				</c:if>
					...
					<thead>
						<tr>
							<th scope="col" class=" smallcol">
								<script>multiChoiceMap["mciu5_0"] = "all";</script>
								<div class="checkbox">
									<input type="checkbox" id="mciu5_0SelectAll" onclick="toggleCheckboxes('mciu5_0', 'Checked')" class="selectAll"/>
								</div>
								<label class="sr-only" for="mciu5_0SelectAll">Select all rows</label>
							</th>
					        <th scope="col" class="">
						     	userName
					        </th>
...
<th scope="col" class=" smallcol"></th> 
				...
				<tbody>
				<c:forEach var="current" varStatus="status" items="${mciu5.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr>	
							<td class=" smallcol">				
							  <div class="checkbox">
							  	<html:multibox styleId="mciu5_0_${index}" property="mciu5Checked" styleClass="checkBox"><c:out value="${mciu5.key[index]}"/></html:multibox>
							  </div>
							  <label class="sr-only" for="mciu5_0_${index}">Select row</label>
							</td>
								<td class=" string">
								...
								<td class=" string">
										<c:out value="${current.direccion}"/>
								</td>
								<td class=" string">
										<c:out value="${current.apellido}"/>
								</td>
								<td class=" string">
										<c:out value="${current.sexo}"/>
								</td>
								<td class=" string">
										<c:out value="${current.telefono}"/>
								</td>
			<c:if test="${webratio:isTargetAccessible('ln30', pageContext)}">
								   		<td class=" smallcol">
											 <c:if test="${webratio:isTargetAccessible('ln30', pageContext)}">
	 		<a title="Modifcar" id="ln30" class="  btn-small" href="<webratio:Link link="ln30" position="index"/>" >
			Modifcar</a>
	</c:if>
										</td>
			</c:if>
			...
			<button title="Eliminar" onclick="$('#page51FormBean')[0].target='_self'" class="btn  btn-default  btn" id="button:ln29" name="button:ln29" type="submit" value="Eliminar">
			Eliminar</button>
	</c:if>
					</div>
			</div>
