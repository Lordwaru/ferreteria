...
<body class="skin-blue fixed">
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<c:if test="${webratio:isTargetAccessible('page46.link', pageContext)}">
									<li class="active treeview">
										<a href="<webratio:Link escapeXml="true" link="page46.link" position="index"/>">
			...
			<c:if test="${webratio:isTargetAccessible('page47.link', pageContext)}">
														<li>
															<a href="<webratio:Link escapeXml="true" link="page47.link" position="index"/>">
																<span>Servicios</span>
															</a>
														</li>
			</c:if>
			...
			<c:if test="${webratio:isTargetAccessible('page49.link', pageContext)}">
									<li class="treeview">
										<a href="<webratio:Link escapeXml="true" link="page49.link" position="index"/>">
			...
			<c:if test="${webratio:isTargetAccessible('page48.link', pageContext)}">
															<li>
																<a href="<webratio:Link escapeXml="true" link="page48.link" position="index"/>">
																	<span>Vision</span>
																</a>
															</li>
			</c:if>
			...
			<c:if test="${webratio:isTargetAccessible('page45.link', pageContext)}">
									<li><a href="<webratio:Link escapeXml="true" link="page45.link"/>">Contacto</a></li>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('lou2.link', pageContext)}">
<c:if test="${webratio:getStartOperation(pageContext) eq 'lou2'}">
									<li class="active"><a href="<webratio:Link escapeXml="true" link="lou2.link"/>">Cerrar sesi&oacute;n</a></li>
								</c:if>
<c:if test="${webratio:getStartOperation(pageContext) ne 'lou2'}">
									<li><a href="<webratio:Link escapeXml="true" link="lou2.link"/>">Cerrar sesi&oacute;n</a></li>
								</c:if>
			</c:if>
                    </ul>
                </section>
                ...
                <section class="content-header clearfix">
                    ...
                    <ol class="breadcrumb">
                    	<li>
						 ...
						 <li><c:if test="${webratio:isTargetAccessible('page46.link', pageContext)}">	
 <a class="" href="<webratio:Link link="page46.link"/>">
   Catalogo/Servicios
</a>
			</c:if>
 </li>
...
<c:if test="${not(empty mssu31) and (mssu31.dataSize gt 0)}">
	<div class="alert alert-info ">
		<span class="glyphicon glyphicon-info-sign"></span>
		<c:forEach var="current" varStatus="status" items="${mssu31.data}">
				...
				<c:forEach var="current" varStatus="status" items="${inu3.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr<c:if test="${inu3.currentIndex eq index}"> class="info"</c:if>>	
								<td class=" string">
									<c:out value="${current.nombre}"/>
								</td>
								<td class=" string">
									<c:out value="${current.descripcion}"/>
								</td>
								<td class=" decimal">
									<fmt:formatNumber value="${current.precio}" pattern="${decimalPattern}"/>
								</td>
						</tr>
					</c:forEach>
					</tbody>
