...
<body class="skin-blue fixed">
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page55.do${wrAjax ? '#!ajax=true' : ''}" styleId="page55FormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page55"/>
...
<c:if test="${webratio:getStartOperation(pageContext) ne 'lou5'}">
 ...
 <a class="" href="<webratio:Link link="page55.link"/>">
   Operaciones de Productos
						...
						<li class="active">Alta de Productos</li>
                    </ol>
                </section>
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu16"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<c:set var="btFieldError"><html:errors property="fld77"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld77" class="control-label col-md-2">Nombre</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld77" styleClass="wr-submitButtons:ln34  form-control " property="fld77" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld77"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld78"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld78" class="control-label col-md-2">Descripci&oacute;n</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld78" styleClass="wr-submitButtons:ln34  form-control " property="fld78" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld78"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld79_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld79_locale" class="control-label col-md-2">Precio</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld79_locale" styleClass="wr-submitButtons:ln34  form-control " property="fld79_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld79_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln34', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln34', pageContext)}">
			<button title="Registrar Producto" onclick="$('#page55FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln34" name="button:ln34" type="submit" value="Registrar Producto">
