...
<body class="skin-blue fixed">
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page54.do${wrAjax ? '#!ajax=true' : ''}" styleId="page54FormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page54"/>
...
<c:if test="${webratio:getStartOperation(pageContext) ne 'lou5'}">
 ...
 <a class="" href="<webratio:Link link="page55.link"/>">
   Operaciones de Productos
						...
						<li class="active">Consulta Productos</li>
                    </ol>
                </section>
...
<c:if test="${not(empty mciu6) and (mciu6.dataSize gt 0)}">
	...
	<div class="box-body">
		<div class="plain ">
			<div class="plain MultiChoiceIndexUnit">
				<c:set var="btFieldError"><html:errors property="mciu6Checked"/></c:set>
				<c:if test="${not (empty btFieldError)}">
					<div class="alert alert-danger alert-dismissable">
						<span class="glyphicon glyphicon-ban-circle"></span>
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
						<html:errors property="mciu6Checked"/>
					</div>
				</c:if>
					...
					<thead>
						<tr>
							<th scope="col" class=" smallcol">
								<script>multiChoiceMap["mciu6_0"] = "all";</script>
								<div class="checkbox">
									<input type="checkbox" id="mciu6_0SelectAll" onclick="toggleCheckboxes('mciu6_0', 'Checked')" class="selectAll"/>
								</div>
								<label class="sr-only" for="mciu6_0SelectAll">Select all rows</label>
							</th>
					        <th scope="col" class="">
						     	nombre
					        </th>
			...
			<c:if test="${webratio:isTargetAccessible('ln33', pageContext)}">
<th scope="col" class=" smallcol"></th> 
			</c:if>
						</tr>
					</thead>
				<tbody>
				<c:forEach var="current" varStatus="status" items="${mciu6.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr>	
							<td class=" smallcol">				
							  <div class="checkbox">
							  	<html:multibox styleId="mciu6_0_${index}" property="mciu6Checked" styleClass="checkBox"><c:out value="${mciu6.key[index]}"/></html:multibox>
							  </div>
							  <label class="sr-only" for="mciu6_0_${index}">Select row</label>
							</td>
								<td class=" string">
			...
			<c:if test="${webratio:isTargetAccessible('ln33', pageContext)}">
								   		<td class=" smallcol">
											 <c:if test="${webratio:isTargetAccessible('ln33', pageContext)}">
	 		<a title="Actualizar" id="ln33" class="  btn-small" href="<webratio:Link link="ln33" position="index"/>" >
			Actualizar</a>
	</c:if>
										</td>
			</c:if>
					...
					<div class="btn-toolbar">
								 <c:if test="${webratio:isTargetAccessible('ln32', pageContext)}">
			<button title="Eliminar producto" onclick="$('#page54FormBean')[0].target='_self'" class="btn  btn-default  btn" id="button:ln32" name="button:ln32" type="submit" value="Eliminar producto">
